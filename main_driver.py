...
def get_args():
    ...
    parser.add_argument(
        "--dim",
        type=int,
        help="Override the feature dimension D of the dataset. Rewrites mu/mus/weight/sigma to match.",
    )
    parser.add_argument(
...
def apply_dim_override(data_kwargs, dim):
    """Rewrite dimension-dependent dataset kwargs in place to be consistent with `dim`.

    The original configs encode `mu`, `mus`, `weight` and `sigma` as Python
    expression strings that get `eval`'d in `datasets.Data`. When the user
    sweeps over multiple D values, those strings must be regenerated so the
    vectors and matrices match the new dimension; otherwise the data
    generators raise shape errors.
    """
    data_kwargs["dim"] = int(dim)
    if "mu" in data_kwargs:
        data_kwargs["mu"] = f"np.zeros({dim})"
    if "mus" in data_kwargs:
        data_kwargs["mus"] = f"[np.ones({dim}), -1 * np.ones({dim})]"
    if "weight" in data_kwargs:
        data_kwargs["weight"] = f"np.arange({dim} - 1, -1, -1)"
    if "sigma" in data_kwargs:
        data_kwargs["sigma"] = f"np.identity({dim})"
...
def process_args(args):
    if args.experiment_json:
        ...
        if args.dim is not None:
            apply_dim_override(args.experiment_json["dataset"]["data_kwargs"], args.dim)
...
if __name__ == "__main__":
    ...
    if not args.no_logs:
        parse_utils.save_experiment(experiment, os.path.join(args.results_dir, "checkpoints"), results)
